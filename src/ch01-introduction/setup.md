...
Rustの公式インストーラである**rustup**を使用することを強く推奨します。rustupは、Rustのバージョン管理、コンパイラやツールチェーンの更新を簡単に行えるツールです。
...
インストーラが起動したら、画面の指示に従います。通常は、デフォルトのオプション（1を入力してEnter）で問題ありません。

#### Windows

[rustup公式サイト](https://rustup.rs/)から、Windows用のインストーラ（`rustup-init.exe`）をダウンロードして実行します。

インストーラを実行すると、Visual Studio C++ Build toolsのインストールが必要であることが通知される場合があります。その場合は、画面の指示に従ってBuild toolsをインストールしてください。これはRustがC/C++のリンカやライブラリを使用するために必要です。

インストールが完了したら、コマンドプロンプトまたはPowerShellを再起動してください。
