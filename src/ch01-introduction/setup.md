...
AI coding agent は、演習用プロジェクトや本書のリポジトリのルートディレクトリで起動し、`Cargo.toml`、`src`ディレクトリ、テスト、差分を参照できる状態にします。agent に任せた変更は、`cargo check`や`cargo test`で確認し、必要に応じて`git status`と`git diff`で変更内容を確認してください。
