...
5. **確認する**
   ...
   `cargo test` は、実装した関数や小さい検証問題が期待通りに動くかを確認するために使います。`git status` と `git diff` は、どのファイルが変更されたかを確認するために使います。

## AI agent を使う場合の標準的な流れ

AI agent を使う場合は、いきなり「この章のコードを実装して」と頼むのではなく、まず問題設定と検証方針を整理させるとよいでしょう。
...
4. **検査する**
   `cargo test`、保存された結果、プロット、`git diff` を確認します。コンパイルが通ることは、物理的・数値的に正しいことを意味しません。
...
本書では、Rustで計算を行い、結果をCSVやJSONなどのファイルに保存し、プロットは保存済みデータから作成する流れを基本にします。
