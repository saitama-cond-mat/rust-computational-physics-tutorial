...
本書では、Rustで計算を行い、結果をCSVやJSONなどのファイルに保存し、プロットは保存済みデータから作成する流れを基本にします。この方法では、入力パラメータ、乱数 seed、実行条件、出力ファイルを明示的に残せるため、結果の再現性や検証がしやすくなります。入出力やプロット用の定型的なコードは、AI agent に自然言語で指示して任せることができます。
