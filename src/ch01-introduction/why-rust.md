...
- **Fortran**：数値計算やHPCの分野で長い実績があり、配列計算や既存の数値ライブラリ資産に強みがあります。一方で、テンソルネットワークのように複雑なデータ構造、抽象化、テスト分割、依存関係管理を伴うアルゴリズムを実装する場合、設計や保守の負担が大きくなりやすい面があります。近年は[fpm (Fortran Package Manager)](https://fpm.fortran-lang.org/)も登場していますが、Cargoのようにビルド、依存関係管理、テストを一体で扱うワークフローはRustの方が統一されています。
...
- **Python**：GIL（Global Interpreter Lock）の制約により、CPU中心の処理を単一プロセス内の複数スレッドで並列化することには制限があります。
...
科学技術計算において、コードの再現性や長期的な保守性は極めて重要です。Rustの統一されたツールチェーンは、これらの維持を強力に支援します。例えば、`cargo new`によるプロジェクト作成、`cargo add`による依存関係の追加、`cargo build`によるビルド、`cargo test`によるテスト実行といった一連の作業が、すべて`cargo`という単一のツールで完結します。`Cargo.toml`ファイルによって依存関係が宣言的に管理されるため、どの環境でも`cargo build`を実行するだけで同一のビルドを再現できます。

#### AI agent 時代における検査の足場

近年は、AI agent を用いてコードの作成や修正を補助する機会が増えています。このときRustが有用なのは、AI agent がRustコードを常に正しく生成できるからではありません。むしろ、Rustの型システム、所有権、借用、明示的な可変性、そして`cargo test`を含むツールチェーンが、生成されたコードを検査するための強い足場になるからです。

C/C++では、aliasing、可変な共有、メモリの寿命、データ競合に起因する問題が、実行時のまれな条件で初めて現れ、一見すると原因の分からないバグになることがあります。Rustでは、これらの多くをコンパイル時に検査し、不適切な参照の使い方やデータの共有を早い段階で排除できます。これは、AI agent が生成したコードを無制限に信用するのではなく、コンパイラとテストによるハーネスの中で扱うという意味で重要です。

ただし、コンパイルが通ることは、物理的・数値的に正しいことを意味しません。数式、単位、境界条件、刻み幅、乱数 seed、保存量、収束性などは、読者自身が問題設定に立ち返って検証する必要があります。本書でRustと計算物理の基礎を学ぶ目的は、AI agent の出力をそのまま受け入れることではなく、検査し、修正し、信頼できる計算へ近づけることにあります。
...
また、Rustはコンパイルが必須であるため、PythonのJupyter Notebookのような環境でコードを断片的に実行し、結果を即座に確認するといったインタラクティブな開発は得意ではありません。その代わり、本書ではRustで計算を行い、結果をCSVやJSONなどのファイルに保存し、プロットは保存済みデータから作成するという流れを基本にします。

この方法では、入力パラメータ、乱数 seed、実行条件、出力ファイルを明示的に残せるため、結果の再現性や検証がしやすくなります。一方で、人間が毎回入出力やプロット用の補助コードを書くのは面倒です。AI agent を利用する場合は、「この計算結果をCSVに保存する関数を追加して」「保存済みデータからプロットするスクリプトを作って」のように自然言語で指示し、定型的な入出力コードの作成を補助させることができます。
