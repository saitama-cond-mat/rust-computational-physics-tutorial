...
> - DFTを行列演算（線形変換）として解釈し、第6章で学んだ知識と結びつける。
...
第4章で学んだ`ndarray`を使用して、定義式に基づいてDFTを計算するコードを実装してみましょう。複素数の扱いは`num-complex`クレート（`Complex64`）を使用します。
...
第6章で学んだように、DFTはベクトルに対する線形変換とみなすことができます。
