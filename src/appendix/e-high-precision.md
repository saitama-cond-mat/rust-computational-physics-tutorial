...
標準の64ビット浮動小数点数 `f64` は、ほとんどの科学技術計算において十分な精度と広い表現範囲を提供します。しかし、[浮動小数点演算と誤差](../ch02-rust-numerics/floating-point.md)で見たように、浮動小数点演算には常に丸め誤差や桁落ちといった問題が伴います。特に、下記のようなケースでは `f64` の精度では不十分となり、計算結果に大きな誤差が生じることがあります。
