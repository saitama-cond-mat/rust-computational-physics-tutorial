...
格子、行列、画像、波動関数、相関関数などを扱うとき、データは多次元の添字を持ちます。しかし、コンピュータのメモリ上では、データは基本的に1次元に並びます。この対応を理解しておくと、配列の扱い、性能、ファイル保存を考えやすくなります。
...
stride は、ある軸に沿って添字を1つ進めたとき、メモリ上で何要素進むかを表します。連続した配列ではアクセスが速く、stride が大きいアクセスでは cache 効率が悪くなることがあります。
