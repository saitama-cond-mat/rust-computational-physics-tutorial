...
- **動的な長さ**: スライスの長さはコンパイル時には決まっておらず、実行時に決まります。
...
// vec!マクロで初期値を指定して作成
let mut positions = vec![[0.0, 0.0], [1.0, 1.0]];
...
- **短所**: ヒープ割り当てに伴うわずかなパフォーマンス上のオーバーヘッド。要素追加時に容量が不足すると、より大きなメモリ領域を確保してデータをコピーする「再割り当て（reallocation）」が発生する可能性があります。
